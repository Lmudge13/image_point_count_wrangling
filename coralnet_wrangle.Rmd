...
######Get the percent covers of all Labels, across all sites in your pointcount dataset:
...
## Percent cover of each label, by site & spur (spur = transect number)
pointcount %>%
  dplyr::group_by(Site, Spur, Label) %>%
  dplyr::summarise (n = n()) %>%
  dplyr::mutate(perc_cov = n / sum(n)*100)

### Create a column in akumal that has the % macro cover for each spur/site combo:
#benthic %>%
  #group_by(site, spur, functional) %>%
  #summarise (n = n()) %>%
  #mutate(perc_cov = n / sum(n)*100) %>%
  #filter(functional=="macro")
  ###no macro count for 161Acrop 1b because that was an elkhorn thicket

macro<- benthic %>%
  group_by(site, spur, functional) %>%
  summarise (n = n()) %>%
  mutate(PC_macro = n / sum(n)*100) %>%
  filter(functional=="macro") %>%
  select(site, spur, PC_macro)
